...
public class RayTracingMaster : MonoBehaviour
{
    ...
    public void SetUpScene()
    {
        ...
        {
            _sphereBuffer = new ComputeBuffer(SceneParser._SceneData._SphereCount, 24);
        ...
        if (SceneParser._SceneData._MeshCount > 0)
        {
            ...
            _MeshDataBuffer = new ComputeBuffer(SceneParser._SceneData._MeshCount, 48);
            _MeshDataBuffer.SetData(SceneParser._SceneData._MeshDataList);
        }

        if (SceneParser._SceneData._MaterialCount > 0)
        {
            _MaterialBuffer = new ComputeBuffer(SceneParser._SceneData._MaterialCount, 72);
            _MaterialBuffer.SetData(SceneParser._SceneData._MaterialDatas);
        }

        if (SceneParser._SceneData._PointLightCount > 0)
        {
            _PointLightBuffer = new ComputeBuffer(SceneParser._SceneData._PointLightCount, 28);
    ...
    public void SetShaderParameters()
    {
        ...
        Vector3 d = SceneParser._SceneData._DirectLightData._DirectionalLightDirection;
        RayTracingShader.SetVector("_DirectionalLightDirection", new Vector4(d.x, d.y, d.z, 1));

        Vector3 l = SceneParser._SceneData._DirectLightData._DirectionalLightColor;
        RayTracingShader.SetVector("_DirectionalLightColor",
                   new Vector4(l.x, l.y, l.z, SceneParser._SceneData._DirectLightData._DirectionalLightIntensity));
